...
export class LaunchController {
  ...
  async resumeSession(claudeSessionId: string): Promise<void> {
    ...
    try {
      ...
      if (!await this.supportsPersistence(executable)) {
        this.reportResumeFailure(claudeSessionId, false, "This Claude executable does not support session resumption.");
        return;
      }
  ...
  private async supportsPersistence(executable: string): Promise<boolean> {
    ...
    this.dependencies.logger.startupError(new Error(
      `Session persistence skipped for ${executable}: unsupported flags or failed capability probe.`
    ));
    return false;
  }

  /** Offers recovery without deleting stale metadata on dismissal. */
  private reportResumeFailure(claudeSessionId: string, rootFailure: boolean, message?: string): void {
