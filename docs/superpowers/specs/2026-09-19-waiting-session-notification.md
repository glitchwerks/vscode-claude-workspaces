...
**Status:** Draft — D5, D8, D9, D10, D11 decided by the user 2026-09-19 and D6 decided 2026-09-20 (see §14). Phase 0 Gate 1 (hook viability) resolved **GO** and Gate 2 (window foreground) resolved **NO-GO** on 2026-09-19 ([Gate 1](https://github.com/glitchwerks/vscode-claude-workspaces/issues/51#issuecomment-5742180727), [Gate 2](https://github.com/glitchwerks/vscode-claude-workspaces/issues/51#issuecomment-5742277872)); Task 0.3 verified that VS Code registers `claudeWorkspaces.sessions.focus` (`test/integration/activation.test.ts:L62`, `test/integration/activation.test.ts:L521-L528`). D12 is resolved not-applicable: Gate 1's merge question (Q2) was GO, so the D1-alt contingency never triggers.
...
**D6 — decided 2026-09-20:** directly bundle KDE SnoreToast 0.9.0, without `node-notifier` or another runtime npm dependency. The executable, LGPL-3 license, upstream source/archive URLs, signer, and pinned SHA-256 are recorded in `media/attention/snoretoast/README.md:L3-L15`; package enumeration and hash verification live at `test/unit/packageAssets.test.ts:L14-L17` and `:L68-L72`. SnoreToast's documented named-pipe callback is the Phase 5 activation seam (https://github.com/KDE/snoretoast#readme, fetched 2026-09-20). This selects the toast-emission layer only; the taskbar-flash fallback from Gate 2 remains the activation behavior.
...
| D6 | Toast emission mechanism | Directly bundle KDE SnoreToast 0.9.0 | **Decided 2026-09-20** — preserve the LGPL-3 license and source/provenance record, pin and test the executable hash, and use the documented named-pipe callback as the Phase 5 activation seam (`media/attention/snoretoast/README.md:L3-L15`) | User |
...
- **User (@cbeaulieu-gt)** — decided D5, D8, D9, D10, D11 on 2026-09-19 and D6 on 2026-09-20 (§14); Gate 1 and Gate 2 both resolved 2026-09-19 ([Gate 1](https://github.com/glitchwerks/vscode-claude-workspaces/issues/51#issuecomment-5742180727), [Gate 2](https://github.com/glitchwerks/vscode-claude-workspaces/issues/51#issuecomment-5742277872)). Task 0.3 is resolved by the generated-command integration guard (`test/integration/activation.test.ts:L521-L528`).
