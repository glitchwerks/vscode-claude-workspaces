...
| **1** | `activity` state contract | Gate 1 passed; D5 decided | Field published to webview; #109/#113 unblocked |
| **2** | Env injection + channel plumbing | Phase 1 merged | Both PTY branches carry the channel vars, proven by test |
| **3** | Hook script, signal ingestion, dedup | Phase 2 merged; D8 decided | Waiting state driven end-to-end by real hooks |
| **4** | Notification emission + focus suppression | Phase 3 merged; D6, D9 decided | Native toast on unfocused window; silent when focused |
| **5** | Click routing: reveal + activate | Phase 4 merged; D10 decided | Selecting a notification lands on the correct session |
| **6** | Docs, configuration, manual runbook | Phase 5 merged; D11 decided | README + settings + runbook merged; #51 closable |
...
**Task 1.1 — Confirm D5 with the user before writing code. Complexity: Low.**
Does "waiting" include `Stop` / `idle_prompt`, or only blocked-on-prompt? Recommendation: blocked-on-prompt only. This choice propagates directly into #113's badge semantics and cannot be cheaply reversed later.
...
Snapshots are immutable and republished through `publishSessions()` (`src/sessions/sessionManager.ts:472-478`); an activity change follows the same path as `rename` (`:340-352`). Sessions removed at `:390-402` must clear activity state.
...
One directory per extension host, created at activation (`src/extension.ts:127-264`), watched by that host only. **Cleanup is part of this task, not deferred** (spec §11.5): on activation, remove channel directories whose owning host no longer exists, or signals leak permanently.
...
Signal → activity transition per spec §5 D2. Unknown matcher values are ignored, not fatal (spec §11.3).

**Task 3.5 — Stage dedup. Complexity: Medium. Depends on D8.**
Per spec §7: one notification per open stage; stage closes on `UserPromptSubmit`, `SessionEnd`, or session removal. **Do not port the reference implementation's `O_EXCL` claim race** — it arbitrates between two notifiers, and this design has only one (spec §7). The deviation is deliberate.
...
`vscode.window.state.focused` / `onDidChangeWindowState`, behind an injected boundary so it is unit-testable — matching how `ExtensionNotificationsApi` (`src/extension.ts:114-117`) and the other VS Code boundaries in this codebase are already injected. Covers G8, plus D9 (do not fire on blur).
...
**Task 5.3 — Handle the gated-view case. Complexity: Low. Depends on D10.**
The view is gated `"when": "claudeWorkspaces.savedWorkspace"` (`package.json:135`); the focus command silently no-ops when false. Fall back to a message naming the session.
...
**Task 6.2 — Settings.** New `claudeWorkspaces.*` properties (`package.json:139-157`), at minimum an enable/disable toggle; plus D11 if `idle_prompt` escalation is separately configurable.
...
| D5 chosen wrong → #113 badge counts nearly every session | Medium | High — cross-issue rework | Decide D5 before Phase 1 code |

---

## 5. Dependencies

- **User decisions** D5 (before Phase 1), D8 (Phase 3), D6 + D9 (Phase 4), D10 (Phase 5), D11 (Phase 6), D12 (only if Gate 1 fails), and the Gate 2 timebox and go/no-go.
